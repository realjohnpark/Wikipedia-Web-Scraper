...
def displayMenu():
    ...
    Displays all contents in the menu to the terminal
...
def isHeader(element):
    ...
    Checks if given element is a header of any type

    '''

    return element.name in ['h2', 'h3', 'h4', 'h5', 'h6']

def elementInDisplay(element, line):
    '''

    Checks if given element is within a line in the menu display

    '''
    
    return element.get_text().strip().replace("[edit]", "") in menuDisplay[line]
...
def findAddHeaderFrequencies(frequenciesList, element, checked, excludeList):
    ...
    for line in range(checked[0], len(menuDisplay)):
        if (elementInDisplay(element, line)): 
...
def isValidLink(element):
    '''

    Checks if given element is a valid link to display

    '''

    return element.has_attr('href') and element.name == 'a'
...
def parseAll():
    ...
    Also initiates findLinks to add all links to correct header sections
    in the display
